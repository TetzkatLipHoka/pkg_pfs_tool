...
static struct {
  ...
  { "ekpfs_obf_key_17", &g_ekpfs_obf_key_17, TYPE_BYTE_ARRAY, 16, 1 },
  { "ekpfs_obf_key_18", &g_ekpfs_obf_key_18, TYPE_BYTE_ARRAY, 16, 1 },
  { "ekpfs_obf_key_19", &g_ekpfs_obf_key_19, TYPE_BYTE_ARRAY, 16, 1 },
...
static int config_cb(void* arg, const char* section, const char* name, const char* value) {
  ...
  } else if (strcasecmp(section, "self_keys") == 0) {
    /* SELF/SPRX root keys. Names: t<N>_rsa = <modulus_hex>,
       t<N>_k<M> = <aes_key_hex>:<aes_iv_hex>  (N=0..3 key generation, M=key index). */
    unsigned int n = 0, m = 0;
    if (sscanf(name, "t%u_k%u", &n, &m) == 2) {
      const char* colon = strchr(value, ':');
      uint8_t* kbuf = NULL;
      uint8_t* ivbuf = NULL;
      size_t ksize = 0, ivsize = 0;
      char keyhex[64];
      if (n >= SELF_ROOT_KEY_TYPE_COUNT || m >= SELF_ROOT_KEY_MAX_KEYS || !colon)
        goto self_invalid_param;
      if ((size_t)(colon - value) >= sizeof(keyhex))
        goto self_invalid_param;
      memcpy(keyhex, value, (size_t)(colon - value));
      keyhex[colon - value] = '\0';
      kbuf = x_to_u8_buffer(keyhex, &ksize);
      ivbuf = x_to_u8_buffer(colon + 1, &ivsize);
      if (!kbuf || !ivbuf || ksize != SELF_ROOT_AES_KEY_SIZE || ivsize != SELF_ROOT_AES_KEY_SIZE) {
        if (kbuf) free(kbuf);
        if (ivbuf) free(ivbuf);
        goto self_invalid_param;
      }
      memcpy(g_self_root_keys[n].aes_key[m], kbuf, SELF_ROOT_AES_KEY_SIZE);
      memcpy(g_self_root_keys[n].aes_iv[m], ivbuf, SELF_ROOT_AES_KEY_SIZE);
      free(kbuf);
      free(ivbuf);
      if (m + 1 > g_self_root_keys[n].key_count)
        g_self_root_keys[n].key_count = m + 1;
    } else if (sscanf(name, "t%u_rsa", &n) == 1) {
      if (n >= SELF_ROOT_KEY_TYPE_COUNT)
        goto self_invalid_param;
      if (g_self_root_keys[n].rsa.n)
        free(g_self_root_keys[n].rsa.n);
      g_self_root_keys[n].rsa.n = strdup(value);
    }
  } else {
...
self_invalid_param:
  warning("Invalid SELF root key parameter '%s'.", name);
  goto done;
