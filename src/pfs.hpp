...
struct pfs_dinode32 {
  uint32_t block_count;                                             // Bytes actually held
  struct pfs_block32 direct_blocks[PFS_DIRECT_BLOCK_MAX_COUNT];     // Direct disk blocks
  struct pfs_block32 indirect_blocks[PFS_INDIRECT_BLOCK_MAX_COUNT]; // Indirect disk blocks
};
CT_SIZE_ASSERT(struct pfs_dinode32, PFS_DINODE32_STRUCT_SIZE );

// For compressed blocks (64-bit).
struct pfs_dinode64 {
  uint64_t block_count;                                             // Bytes actually held
  struct pfs_block64 direct_blocks[PFS_DIRECT_BLOCK_MAX_COUNT];     // Direct disk blocks
  struct pfs_block64 indirect_blocks[PFS_INDIRECT_BLOCK_MAX_COUNT]; // Indirect disk blocks
};
...
TYPE_BEGIN(struct pfs_dinode, PFS_DINODE_STRUCT_SIZE);
  uint16_t mode;                                                            // IFMT, permissions
...
int pfs_info(struct pfs* pfs, unsigned char list, unsigned char dump_sfo, unsigned char backport, const char* sdk_version);
