...
static inline size_t self_entry_segment_index(struct self_entry* entry) {
  ...
  /* segment_idx is a 16-bit field at bit 20 (matches attr >> 20). Read it with
     an explicit shift: the `segment_idx:16` bitfield crosses the 32-bit storage
     unit boundary and MSVC would place it at bit 32 instead of bit 20. */
  return (size_t)((LE64(entry->flags.bitmask) >> 20) & 0xFFFF);
}
